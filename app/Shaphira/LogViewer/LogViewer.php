<?php namespace Shaphira\LogViewer;
...
class LogViewer {
    ...
    public static function getLogs($level = 'all') {
        $Viewer = new LogViewer();
        $logs = array();

        $pattern = "/\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\].*/";
        $file = File::get($Viewer->path);
        ...
        foreach ($logData as $k => $v) {
            ...
            if($level == "all") {
                ...
                $logs[] = array('log' => trim($v), 'stack' => trim($stack[$k]), 'type' => array_search($type, $Viewer->levels));

            } else {

                if($level == array_search($type, $Viewer->levels))
